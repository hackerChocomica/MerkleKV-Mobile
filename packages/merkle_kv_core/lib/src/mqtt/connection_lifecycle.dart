...
class DefaultConnectionLifecycleManager implements ConnectionLifecycleManager {
  ...
  DefaultConnectionLifecycleManager({
    ...
    Duration? disconnectionTimeout,
        ...
        _disconnectionTimeout =
            disconnectionTimeout ?? const Duration(seconds: 10),
  ...
  Future<void> connect() async {
    ...
    _logger.info(
        '🚀 Starting connection to ${_config.mqttHost}:${_config.mqttPort} (clientId=${_config.clientId})');
    ...
    try {
      ...
      await _mqttClient.connect().timeout(
        ...
        onTimeout: () {
          ...
          throw Exception(
              'Connection timeout after ${_config.keepAliveSeconds * 2} seconds');
        },
      );
  ...
  Future<void> disconnect({bool suppressLWT = true}) async {
    ...
    _logger
        .info('🔌 Starting graceful disconnection (suppressLWT: $suppressLWT)');
  ...
  Future<void> _handleAppBackgrounding() async {
    ...
    _logger.debug(
        'App backgrounding, maintain connection: $_maintainConnectionInBackground');
  ...
  Future<void> _performResourceCleanup() async {
    ...
    try {
      ...
      _metrics?.recordCleanupOperationMetric(
